...
embedding_model = HuggingFaceEmbeddings( # This class is used to generate embeddings for the text chunks.
        model_name=embedding_model_path, # Specifies the path to the pre-trained embedings model used for generating embeddings.
...
## Create the vector store by using given embadding model
...
```python
# Deleting collection after tuttorial
...
**Previous**: [Lang Chain LLM Model](../advanced-langchain/lang-chain-llm-model.md) | **Next**: [Prepare Data](../genai-demos/cv-asistant/prepare-data.md)
