...
assert user, "Please enter your username."
assert password, "Please enter your password."
```

```python
milvus_uri = f"http://{milvus_host}"
milvus_token = f"{user}:{password}"  # Change this after installation
```
...
```python
connections.connect(
    alias="default", host=milvus_host, port=milvus_port, token=milvus_token
)
...
**Previous**: [Qdrant](qdrant.md) | **Next**: [Relational Databases > Build](../relational-databases/build.md)
