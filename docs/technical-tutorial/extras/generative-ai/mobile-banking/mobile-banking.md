...
answer = query_pdf(retriever = retriever_pdf, 
                   question="How can I send money using IBAN", 
                   api_url = api_url,
                   api_token = token)
print(answer)
```
...
**Previous**: [Lang Chain LLM Model](../advanced-langchain/lang-chain-llm-model.md) | **Next**: [LLM Apps > API LLM Apphost > Build](../llm-apps/api-llm-apphost/build.md)
